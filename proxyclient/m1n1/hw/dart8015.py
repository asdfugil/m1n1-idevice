...
class DART8015Regs(RegMap):
    ...
    TCR             = irange(0x100, 4, 4), R_TCR
    TTBR            = (irange(0x200, 4, 16), range(0, 16, 4)), R_TTBR

PTE_TYPES = {
    "dart,t8015": PTE_T8015,
}

class DART8015(Reloadable):
    PAGE_BITS = 12
    ...
    def __init__(self, iface, regs, util=None, compat="dart,t8015"):
        self.iface = iface
        self.regs = regs
        self.u = util
        self.pt_cache = {}
    ...
    def iomap_at(self, stream, iova, addr, size):
        ...
        for page in range(start_page, end_page, self.PAGE_SIZE):
            ...
            if not l1pte.VALID:
                ...
                l1pte = self.ptecls(
                    OFFSET=l2addr >> self.PAGE_BITS, VALID=3)
            ...
            self.pt_cache[l2addr][l2idx] = self.ptecls(
                OFFSET=paddr >> self.PAGE_BITS, VALID=3).value
